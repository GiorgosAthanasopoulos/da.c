...
DA *da_init(DA_Type type) { return da_init_cap(type, DA_DEFAULT_CAPACITY); }
...
void da_extend(DA *da) { da_extend_cap(da, DA_DEFAULT_CAPACITY); }
...
void *da_front(DA *da) { return da_get(da, 0); }

void *da_back(DA *da) { return da_get(da, da->size - 1); }
...
void da_reserve(DA *da, int capacity) { da_extend_cap(da, capacity); }
...
void da_clear_shrink(DA *da) { da_clear_shrink_cap(da, DA_DEFAULT_CAPACITY); }
